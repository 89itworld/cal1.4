...
import{WEBSITE_NAME, WEBSITE_URL} from "@lib/config/constants";
// headline={t("no_status_bookings_yet", { WEBSITE_NAME: t(WEBSITE_NAME) })}
...
function IframeEmbedContainer() {
  ...
  return (
    <>
      <ShellSubHeading title={t("iframe_embed")} subtitle={t("embed_calcom", { WEBSITE_NAME: t(WEBSITE_NAME) })} className="mt-10" />
        ...
        <>
          ...
          <List>
            {data.conferencing.items.map((item) => (
              (item.title==='Google Meet' || item.title==='Zoom Video')?
              <IntegrationListItem
                key={item.title}
                title={item.title}
            ...
            <div className={classNames("flex w-full flex-1 items-center space-x-2 p-3")}>
              ...
              <div className="flex-grow truncate pl-2">
                <ListItemTitle component="h3">
                  MetaMask (
                  <a className="text-blue-500" target="_blank" href={`${WEBSITE_URL}/web3`} rel="noreferrer">
                    Read more
                  </a>
                  )
                </ListItemTitle>
    ...
    <Shell heading={t("installed_apps")} subtitle={t("manage_your_connected_apps")}>
      <AppsShell>
        <ClientSuspense fallback={<Loader />}>
          ...
          <WebhookListContainer title={t("webhooks")} subtitle={t("receive_cal_meeting_data",{ WEBSITE_NAME: t(WEBSITE_NAME) })} />
