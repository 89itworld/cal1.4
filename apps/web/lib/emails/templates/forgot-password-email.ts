...
export default class ForgotPasswordEmail {
  ...
  protected getNodeMailerPayload(): Record<string, unknown> {
    return {
      ...
      from: `${WEBSITE_NAME} <${this.getMailerOptions().from}>`,
      subject: this.passwordEvent.language("reset_password_subject",{ WEBSITE_NAME: this.passwordEvent.language(WEBSITE_NAME) }),
  ...
  protected getTextBody(): string {
    return `
${this.passwordEvent.language("reset_password_subject",{ WEBSITE_NAME: this.passwordEvent.language(WEBSITE_NAME)})}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.passwordEvent.language("reset_password_subject",{ WEBSITE_NAME: this.passwordEvent.language(WEBSITE_NAME)});
    return `
    <!doctype html>
                          ...
                          <td align="left" style="font-size:0px;padding:10px 25px;word-break:break-word;">
                            <div style="font-family:Roboto, Helvetica, sans-serif;font-size:16px;font-weight:500;line-height:1;text-align:left;color:#3E3E3E;">
                              ...
                              <div style="line-height: 6px;">
                                <p style="font-weight: 400; line-height: 24px;">${this.passwordEvent.language(
                                  "have_any_questions"
                                )} <a href="mailto:support@hiboxscheduler.com" style="color: #3E3E3E" target="_blank">${this.passwordEvent.language(
      "contact_our_support_team"
    )}</a></p>
                              </div>
                            </div>
